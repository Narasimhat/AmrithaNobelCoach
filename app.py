...
def render_hero_profile(profile: Optional[dict]) -> None:
    ...
    with st.form("hero-profile-form", clear_on_submit=False):
        ...
        focus_goal_input = st.slider(
            ...
            help="This controls the coach focus timer and bonus points.",
        )
...
def render_coach_tab(client: OpenAI, profile: Optional[dict], default_api_key: Optional[str]) -> None:
    ...
    with col_chat:
        focus_goal_minutes = (
            int((profile or {}).get("focus_goal_minutes") or FOCUS_BLOCK_MINUTES)
        )
        focus_reward_points = max(
            FOCUS_BLOCK_REWARD,
            int(round(focus_goal_minutes * FOCUS_POINTS_PER_MINUTE)),
        )
        focus_seconds = focus_goal_minutes * 60
        ...
        with st.container():
            ...
            st.caption(
                f"Goal: {focus_goal_minutes} min • Bonus: +{focus_reward_points} points • "
                f"Claimed: {today_focus_claims}/{FOCUS_BLOCKS_PER_DAY}"
            )
